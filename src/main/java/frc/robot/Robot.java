...
public class Robot extends TimedRobot {
  ...
  private static final int climberLockMotorPort = 2;
  ...
  // Motor Locking
  private static final int ticksForClimbLockMovement = 18;
  ...
  public void teleopPeriodic() {
    ...
    if (secondaryJoystick.getRawButton(1)) {
      rightShooterMotor.set(-.51);
      leftShooterMotor.set(.52);
    } else {
    ...
    // Climber
    if (secondaryJoystick.getRawAxis(2) > .9 && !lockEngaged) {
      climberMotor.set(1);
    } else if (secondaryJoystick.getRawAxis(3) > .9 && !lockEngaged) {
      climberMotor.set(-1);
    } else {
      climberMotor.set(0);
    }
    ...
    if (ticksOfLockMovement != -1) {
      ...
      if (ticksOfLockMovement == ticksForClimbLockMovement) {
        lockEngaged = !lockEngaged;
        ticksOfLockMovement = -1;
        climberLockMotor.set(0);
      }
    } else if (secondaryJoystick.getPOV() == 0) {
      ticksOfLockMovement = 0;
    }
    ...
    if (secondaryJoystick.getRawButton(4)) {
      ...
      final Number targetOffsetAngle_Horizontal = limelightTable.getEntry("tx").getNumber(0);
